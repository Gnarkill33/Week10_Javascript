...
/*const calculateProduct = function calculateProduct(a, b) {
	return a * b;
}
...
/*let rabbit = "Кролик"; Почему не работает с console внутри функции????????????????????????????????????????????
let cat = "Котик";
...
/*const currentYear = 2023; Зачем мы задаем здесь переменные??????????????????????????????????????????7
...
console.log(
	'Пользователю ' + calculateAge (2023, 1990) + ' лет'
)*/
...
function calculateMyAge(myBirthYear) {
	return currentYear - myBirthYear;
}

let result = calculateMyAge(1987);
console.log(result);*/
